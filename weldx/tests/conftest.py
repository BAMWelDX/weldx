...
def single_pass_weld_asdf(request):
    """returns an example of a single pass weld fulfilling the single_pass_weld schema.

    Returns
    -------
    buff :
        a io.BytesIO instance containing the binary output of the ASDF serialization.
    """
    ...
    from weldx.asdf.util import write_buffer, get_schema_path
    ...
    rot = WXRotation.from_euler(seq="x", angles=Q_(180, "°"), degrees=True)
    ...
    buff = write_buffer(tree=tree, asdffile_kwargs=dict(custom_schema=model_path))
