...
Through the given shape the variable `n` is defined. And any shape that does not match our requirement would not be
...
gets triggered by the `wx_shape` keyword.
...
and `...`. But be aware that the scope of this "inline" wx_shape validation is limited to the property that it
validates!
...
properties in its scope. To assure `array_prop` and `array_prop2` have the same shape we use the following schema:
...
Properties that are optional (not listed as `required`) must be indicated as such for shape validation by putting the
name in brackets. In this example, both `optional_prop` will only get validated if it exists in the tree.
...
The following custom types can be validate with `wx_shape` even though the might not always define a shape property in
itself.
